...
def kb_chat(api: ApiRequest):
    ...
    with st.sidebar:
        ...
        with tabs[0]:
            ...
            prompt_name = "default"
            kb_top_k = st.number_input("匹配知识条数：", 1, 20, Settings.kb_settings.VECTOR_SEARCH_TOP_K, key="kb_top_k")
            score_threshold = st.slider("知识匹配分数阈值：", 0.0, 1.0, float(Settings.kb_settings.SCORE_THRESHOLD), step=0.1, key="score_threshold")
